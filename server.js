...
app.post('/api/schedule-call', authenticateToken, (req, res) => {
    ...
    try {
        ...
        console.log(`💳 Deducted 1 credit from user ${user.name} (ID: ${user.id}). Remaining credits: ${user.credits}`);
...
class CallScheduler {
    ...
    scheduleCall(callData) {
        try {
            ...
            // Handle both 'time' and 'scheduledTime' fields
            const timeField = callData.time || callData.scheduledTime;
            
            if (!timeField) {
                console.error(`❌ No time field found for call ${callData.name}`);
                return null;
            }
            ...
            if (timeField instanceof Date) {
                ...
                if (timeString && timeString.includes('T') && timeString.includes('Z')) {
                    ...
                    console.error(`❌ Invalid time format for call ${callData.name}: ${timeField}`);
                    return null;
                }
            }
            ...
            if (callTime > now) {
                ...
                // Call is due now or in the past - don't schedule, let checkScheduledCalls handle it
                console.log(`⏰ Call for ${callData.name} is due now or in the past - will be handled by scheduler check`);
                return null;
            }
        } catch (error) {
    ...
    async makeCall(callData) {
        try {
            ...
            console.log(`⚠️ Call to ${callData.name} failed but server continues running`);
            return null;
        }
    }
    ...
    checkScheduledCalls() {
        ...
        for (const call of callsData.calls) {
            ...
            if (call.completed || call.failed || call.status === 'completed') {
                console.log(`   ⏭️  Skipping call ${call.id} (${call.name}) - Status: ${call.status || 'completed'}`);
                continue;
            }

            // Check if call has been in-progress for too long (more than 10 minutes)
            if (call.status === 'in-progress' && call.started_at) {
                const startedTime = new Date(call.started_at);
                const timeInProgress = now - startedTime;
                const maxProgressTime = 10 * 60 * 1000; // 10 minutes in milliseconds
                
                if (timeInProgress > maxProgressTime) {
                    console.log(`   ⏰ Call ${call.id} has been in progress for ${Math.floor(timeInProgress / 60000)} minutes, marking as failed`);
                    call.failed = true;
                    call.status = 'failed';
                    call.failed_at = new Date().toISOString();
                    call.error = 'Call timeout - no status update received';
                    saveCalls(callsData);
                    updated = true;
                    continue;
                } else {
                    console.log(`   🔄 Call ${call.id} is in progress (${Math.floor(timeInProgress / 60000)} minutes elapsed)`);
                    continue;
                }
            }
            ...
            // Handle both 'time' and 'scheduledTime' fields
            const timeField = call.time || call.scheduledTime;
            
            if (!timeField) {
                console.log(`   ⚠️  No time field found for call ${call.name}, skipping`);
                continue;
            }
            ...
            if (timeField instanceof Date) {
                ...
                if (timeString && timeString.includes('T') && timeString.includes('Z')) {
                    ...
                    console.log(`   ⚠️  Invalid time format for call ${call.name}: ${timeField}, skipping`);
                    continue;
                }
            }
            ...
            if (callTime <= now) {
                ...
                // If call is already in-progress, don't do anything
                if (call.status === 'in-progress') {
                    console.log(`   🔄 Call ${call.id} is already in progress, skipping`);
                    continue;
                }
                
                // If call is scheduled or pending, make the call
                if (!call.status || call.status === 'Scheduled' || call.status === 'pending') {
                    ...
                    // For any other status, mark as failed if it's past due
                    if (!call.failed) {
                        call.failed = true;
                        call.status = 'failed';
                        call.failed_at = new Date().toISOString();
                        saveCalls(callsData);
                        console.log(`   ❌ Marked call ${call.id} as failed (missed)`);
                        updated = true;
                    }
                }
            } else {
...
app.post('/twiml/ask', express.urlencoded({ extended: false }), (req, res) => {
    try {
        ...
        // Find the call to get the name
        const callsData = loadCalls();
        const call = callsData.calls.find(c => c.twilio_call_sid === callSid);
        const userName = call ? call.name : 'User';
        ...
        if (questionIndex === 0) {
            ...
            response.say(`Thank you ${userName}, we will contact you soon.`);
            response.hangup();
...
app.post('/api/scheduler/cancel/:callId', authenticateToken, (req, res) => {
    try {
        ...
        if (success) {
            // Check if we should refund credits
            const callsData = loadCalls();
            const call = callsData.calls.find(c => c.id == callId);
            
            if (call && call.userId === req.user.userId) {
                // Only refund if call hasn't been made yet (not in-progress, completed, or failed)
                if (!call.status || call.status === 'Scheduled' || call.status === 'pending') {
                    const usersData = loadUsers();
                    ...
                    if (user) {
                        ...
                        console.log(`💳 Refunded 1 credit to user ${user.name} (ID: ${user.id}) for cancelled call. New balance: ${user.credits}`);
                        
                        res.json({ 
                            success: true, 
                            message: 'Call cancelled successfully. 1 credit refunded.',
                            refundedCredits: 1,
...
app.get('/api/responses', authenticateToken, (req, res) => {
    try {
        ...
        if (req.user.role !== 'admin') {
            return res.status(403).json({ 
                success: false, 
                message: 'Only admins can view all responses' 
            });
        }
        ...
        // Enhance responses with user and call information
...
// Get user-specific responses (for regular users)
app.get('/api/user-responses', authenticateToken, (req, res) => {
    try {
        ...
        // Filter responses to only include those from the current user's calls
        ...
        // Enhance responses with user and call information
        const enhancedResponses = userResponses.map(response => {
            // Find corresponding call data
            const call = callsData.calls.find(c => c.twilio_call_sid === response.callSid);
            const user = usersData.users.find(u => u.id === (call ? call.userId : null));
        ...
        res.status(500).json({ 
            success: false, 
            message: 'Failed to fetch user responses: ' + error.message 
        });
    }
});
...
// Delete individual call (admin only)
...
// Delete user's own call (users can delete their own calls)
app.delete('/api/user-calls/:callId', authenticateToken, (req, res) => {
    try {
        const callId = req.params.callId;
        const callsData = loadCalls();
        const callIndex = callsData.calls.findIndex(c => c.id == callId);
        ...
        // Check if we should refund credits (only if call hasn't been made yet)
        ...
        if (!call.status || call.status === 'Scheduled' || call.status === 'pending') {
            ...
            if (user) {
                ...
                console.log(`💳 Refunded 1 credit to user ${user.name} (ID: ${user.id}) for deleted call. New balance: ${user.credits}`);
            }
        }
        ...
        // Save updated calls
        if (saveCalls(callsData)) {
            console.log(`🗑️ User ${req.user.userId} deleted call ${callId} successfully`);
            
            const response = { 
                success: true, 
                message: refundedCredits > 0 ? 'Call deleted successfully. 1 credit refunded.' : 'Call deleted successfully'
            };
            
            if (refundedCredits > 0) {
                response.refundedCredits = refundedCredits;
        ...
        console.error('Error deleting user call:', error);
        res.status(500).json({ 
            success: false, 
            message: 'Failed to delete call: ' + error.message 
        });
